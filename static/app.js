...
const state = {
	...
	cacheStates: {},
	settings: { prefetchCount: 1, coverQuality: "1200", musicbrainzContact: "" },
	trackCache: new Map(),
...
async function api(path, options = {}) {
	...
	try {
		...
		if (quiet) {
			const failure = new AppError(
				"The music service is unreachable.",
				true,
				"quiet",
			);
			failure.quiet = true;
	...
	if (!response.ok) {
		...
		const appError = new AppError(
			failure?.message || body?.detail || `Request failed (${response.status})`,
			failure?.retryable,
			failure?.code,
		);
...
function showView(id) {
	for (const view of ["lock-view", "telegram-view", "app-shell"])
		$(view).hidden = view !== id;
}
...
function updateAmbientArtwork(track) {
	...
	const version =
		track.metadata?.artworkPath || track.artworkVersion || "telegram";
	...
	image.onerror = () => {
		if (request === ambientArtworkRequest)
			layer.style.removeProperty("background-image");
	};
	image.src = url;
}
...
function playerSnapshot() {
	const window = snapshotWindow(state.queue, state.queueIndex, {
		...
		truncated: state.queueTruncated,
	...
	return {
		...
		temporarySource: state.temporarySource,
		panel:
			document.querySelector(".now-tabs .active")?.id?.replace("-tab", "") ||
			"lyrics",
...
function restoreSleepTimer() {
	...
	if (!select) return;
	const remaining =
		(Number(localStorage.getItem("tm-sleep-ends")) || 0) - Date.now();
...
function persistPlayerState() {
	...
	try {
		localStorage.setItem("tm-player-state", JSON.stringify(playerSnapshot()));
	} catch {}
}
...
async function restorePlayerState(saved) {
	...
	try {
		...
		if (
			state.queueIndex < 0 ||
			state.queue[state.queueIndex] !== saved.currentKey
		) {
		...
		audio.addEventListener(
			...
			() => {
				audio.currentTime = Math.min(
					saved.position,
					Number.isFinite(audio.duration) ? audio.duration : saved.position,
				);
			},
			{ once: true },
		);
...
function applyPreferences() {
	...
	document
		.querySelectorAll('[data-setting="theme"] [data-value]')
		.forEach((button) => {
	...
	setSidebarCollapsed(localStorage.getItem("tm-sidebar") === "collapsed", {
		persist: false,
	});
	document.documentElement.style.setProperty(
		"--rail-width",
		`${Math.max(220, Math.min(420, Number(localStorage.getItem("tm-rail-width")) || 260))}px`,
	);
	document.documentElement.style.setProperty(
		"--panel-width",
		`${Math.max(300, Math.min(640, Number(localStorage.getItem("tm-panel-width")) || 368))}px`,
	);
	updateModes();
}
...
async function loadCountries() {
	...
	try {
		...
		for (const country of countryList) {
			country.search = `${country.name} ${country.iso2}`
				.normalize("NFD")
...
function matchCountries(query) {
	...
	for (const country of countryList) {
		...
		if (digits && country.dialCode.startsWith(digits))
			score = score === -1 ? 2 : Math.min(score, 2);
...
function moveCountryActive(step) {
	...
	countryActive =
		(countryActive + step + countryMatches.length) % countryMatches.length;
	syncCountryActive();
}
	...
	qrTimer = setTimeout(async () => {
		...
		try {
			...
			if (status.state === "error") {
				...
				return showError(
					new AppError(status.error || "QR login stopped."),
					startQr,
				);
			}
			pollQr(flowId);
...
async function submitPhone(event) {
	...
	try {
		...
		const flow = await api("/api/telegram/phone", {
			method: "POST",
...
async function submitPhonePassword(event) {
	...
	try {
		button.setAttribute("aria-busy", "true");
		button.disabled = true;
		const status = await api("/api/telegram/password", {
			method: "POST",
			body: JSON.stringify({
				flowId: state.flow,
				password: $("telegram-password").value,
			}),
		});
		await applyPhoneStatus(status);
	} catch (error) {
...
async function boot() {
	...
	if (!status.telegram.linked) {
		...
		if (status.startupError)
			showError(
				new AppError(
					"The saved Telegram authorization expired. Reconnect with QR or phone; your local library and edits are safe.",
				),
				null,
				"Reconnect Telegram",
			);
	...
	let saved = null;
	try {
		saved = normalizePlayerState(
			JSON.parse(localStorage.getItem("tm-player-state") || "null"),
		);
	} catch {}
	const stats = await api("/api/library/stats");
	[state.settings, state.sources] = await Promise.all([
		api("/api/settings"),
		api("/api/sources"),
	]);
...
function renderSources() {
	...
	$("all-count").textContent =
		state.allMusicTotal === null ? "—" : state.allMusicTotal.toLocaleString();
	...
	const temporary =
		state.temporarySource &&
		!state.sources.some((item) => item.chatId === state.temporarySource.chatId)
	...
	const selected = state.likedMode
		? null
		: state.sources.find((item) => item.chatId === state.source) ||
	...
	const synced = formatSyncedAt(
		selected?.lastSyncedAt,
		Math.floor(Date.now() / 1000),
	);
	$("library-summary").textContent =
		`${state.totalTracks.toLocaleString()} ${state.totalTracks === 1 ? "track" : "tracks"}${synced ? ` · ${synced}` : ""}`;
	...
	if (state._sourceChangeScroll) {
		document
			.querySelector(".source-entry.active")
...
const coverObserver = new IntersectionObserver(
	(entries) => {
		for (const entry of entries) {
			...
			if ($("library").classList.contains("is-scrolling"))
				pendingCovers.add(image);
...
function visibleDayBreaks() {
	return !state.source && !state.likedMode && state.sort === "posted"
		? state.dayBreaks
		: [];
}
...
function renderDaySeparator(dayBreak) {
	...
	return label
		? `<div class="day-separator utility" data-day-key="${dayBreak.dayKey}" role="separator">${label}</div>`
		: "";
}
...
function renderTracks(force = false) {
	...
	for (const cell of document.querySelectorAll(".track-head [data-sort]")) {
		...
		if (active)
			cell.setAttribute(
				"aria-sort",
				state.sort === "title" || state.sort === "artist"
					? "ascending"
					: "descending",
			);
	...
	if (force || state.windowStart !== start) {
		...
		const rows = Array.from({ length: end - start }, (_, offset) => {
			...
			const separator = breaksByIndex.has(index)
				? renderDaySeparator(breaksByIndex.get(index))
				: "";
			return (
				separator +
				(state.tracks[index]
					? renderTrackRow(state.tracks[index], index)
		...
		list
			.querySelectorAll(".row-art[data-src]")
	...
	for (let offset = Math.floor(start / 100) * 100; offset < end; offset += 100)
		loadPage(offset);
}
...
async function loadPage(offset, force = false, token = libraryRequest) {
	...
	try {
		const raw =
			(!force && state.libraryCache.get(cacheKey)) ||
			(await api(
				`/api/tracks?${cacheKey}${knownTotalParam(offset, state.totalTracks)}`,
				{ signal: requestController.signal },
			));
		if (token !== libraryRequest) return;
		const page = normalizeTrackPage(raw);
		if (!state.libraryCache.has(cacheKey))
			cacheSet(state.libraryCache, cacheKey, page, 8);
		state.totalTracks = page.total;
		state.allMusicTotal = page.allMusicTotal;
		state.dayBreaks = page.dayBreaks;
		mergePageInto(state.tracks, page, (track) =>
			cacheSet(state.summaryCache, track.key, track, 500),
		);
		...
		state.pageRequests.delete(offset);
	}
}
	...
	try {
		if (force) {
			const [sources, stats] = await Promise.all([
				api("/api/sources", { signal: requestController.signal }),
				api("/api/library/stats", { signal: requestController.signal }),
			]);
...
async function selectSource(chatId) {
	...
	if (
		state.temporarySource &&
		chatId !== state.temporarySource.chatId &&
		state.current?.source?.chatId !== state.temporarySource.chatId
	) {
		if (state.temporaryJob?.jobId)
			api(`/api/jobs/${encodeURIComponent(state.temporaryJob.jobId)}`, {
				method: "DELETE",
			}).catch(() => {});
		state.temporarySource = null;
		state.temporaryJob = null;
	}
...
async function selectTemporary() {
	...
	state.temporaryJob = await api(
		`/api/sources/${encodeURIComponent(state.source)}/preview`,
		{ method: "POST" },
	);
...
function closeGlobalSearch({ clear = false } = {}) {
	...
	if (
		...
		globalResultsCloseTimer = setTimeout(() => {
...
function renderGlobalSearch(message = "") {
	...
	$("global-results-title").textContent =
		$("global-search").value.trim() || "Search everywhere";
	const found = state.globalTracks.length + state.globalSources.length;
	$("global-results-count").textContent = message
		? ""
		: found === GLOBAL_SEARCH_LIMIT
			? `First ${found} results`
...
async function searchEverywhere() {
	...
	try {
		...
		const remote = await api("/api/search/telegram", {
			method: "POST",
			signal: globalController.signal,
			body: JSON.stringify({ query, limit: GLOBAL_SEARCH_LIMIT }),
		});
		...
		for (const track of state.globalTracks)
			cacheSet(state.summaryCache, track.key, track, 500);
		renderGlobalSearch();
	} catch (error) {
		if (error.name !== "AbortError") renderGlobalSearch(error.message);
	} finally {
		if (token === globalRequest) $("global-search-signal").hidden = true;
	}
}
...
async function getTrack(key) {
	...
	cacheSet(state.trackCache, key, track, 100);
	return track;
}
...
async function togglePlayback() {
	...
	if (state.buffering || !audio.paused) {
		audio.pause();
		setBuffering(false);
...
async function playKey(key, queue = null, explicitIndex = null) {
	if (
		...
		(explicitIndex == null || explicitIndex === state.queueIndex)
	) {
	...
	if (state.current && state.current.key !== key && !state.current.qualified)
		api("/api/playback/events", {
			method: "POST",
			body: JSON.stringify({ key: state.current.key, event: "skipped" }),
		}).catch(() => {});
	...
	api("/api/playback/cache-current", {
		method: "POST",
	...
	api("/api/playback/events", {
		method: "POST",
		body: JSON.stringify({ key, event: "started" }),
	}).catch(() => {});
	loadLyrics();
	schedulePersist();
	try {
...
function detailRowsFor(track) {
	...
	return [
		...
		[
			"Format",
			(track.file?.mimeType || "").replace(/^audio\//, "").toUpperCase(),
		],
		["File", track.file?.name],
		[
			"Size",
			track.file?.size ? `${(track.file.size / 1048576).toFixed(1)} MB` : "",
		],
	]
		.filter(([, value]) => value)
		.map(
			([key, value]) =>
				`<div><dt>${key}</dt><dd class="${dataKeys.has(key) ? "detail-data" : ""}">${escapeHtml(value)}</dd></div>`,
		)
		.join("");
}

window.__renderDetailsForTest = (track) => {
	$("track-details").innerHTML = detailRowsFor(track);
};
...
function setTrackUi() {
	...
	if (changed)
		$("playback-status").textContent = `Now playing ${title} by ${artist}.`;
	for (const id of ["mini-art", "large-art"]) {
		...
		placeholder.classList.remove("is-covered");
		const bust = `v=${encodeURIComponent(metadata.artworkPath || "telegram")}`;
		image.onload = () =>
			requestAnimationFrame(() => {
				image.classList.add("is-ready");
				placeholder.classList.add("is-covered");
			});
		if (id === "large-art") {
	...
	$("like-current")
		.querySelector("use")
		.setAttribute("href", liked ? "#i-heart-filled" : "#i-heart");
	$("like-current").setAttribute("aria-pressed", String(liked));
	$("like-current").setAttribute(
		"aria-label",
		liked ? "Unlike current track" : "Like current track",
	);
	...
	document
		.querySelector(`.track-row[data-track-key="${CSS.escape(track.key)}"]`)
		?.classList.add("current");
	if (changed && !matchMedia("(prefers-reduced-motion: reduce)").matches) {
		for (const element of [
			$("player-locate"),
			document.querySelector(".now-title"),
		]) {
	...
	if ("mediaSession" in navigator) {
		navigator.mediaSession.metadata = new MediaMetadata({
			...
			album: metadata.album,
			artwork: [
				{ src: mediaUrl(track, "cover"), sizes: "512x512", type: "image/jpeg" },
			],
		});
		try {
...
function updateTransport() {
	...
	$("play").setAttribute(
		"aria-label",
		state.buffering ? "Pause while loading" : playing ? "Pause" : "Play",
	);
	const row =
		state.current &&
		document.querySelector(
			`.track-row[data-track-key="${CSS.escape(state.current.key)}"]`,
		);
	if (row) {
		row.classList.toggle("buffering", state.buffering);
		row.querySelector(".track-play-overlay").innerHTML = icon(
...
function updateProgress() {
	...
	$("progress").style.setProperty(
		"--buffered",
		`${Math.max(played, bufferedPercent(audio.buffered, duration))}%`,
	);
...
async function loadLyrics(refresh = false) {
	...
	$("lyrics-lines").innerHTML =
		'<div class="lyrics-skeleton"><span></span><span></span><span></span><span></span></div>';
	$("lyrics-empty").hidden = true;
	try {
		const lyrics = await api(
			`${mediaUrl(state.current, "lyrics")}${refresh ? "?refresh=true" : ""}`,
			{ signal: lyricsController.signal, quiet: true },
		);
		...
		if (error.name !== "AbortError" && !error?.quiet) {
			$("lyrics-empty").hidden = true;
			showError(error, () => loadLyrics(refresh));
		}
	}
}
...
function renderLyrics() {
	...
	$("lyrics-lines").innerHTML = lines.length
		? lines
				.map(
					(line, index) =>
						`<button class="lyric-line" type="button" data-lyric="${index}">${escapeHtml(line.text)}</button>`,
				)
				.join("")
	...
	$("lyrics-empty").hidden = !empty;
	$("add-lyrics-empty").hidden = !empty;
	if (empty) $("lyrics-empty").textContent = "No lyrics found for this track.";
}

function updateLyric() {
	const lines = state.lyrics?.lines || [];
	const index = lyricIndex(lines, audio.currentTime * 1000);
	if (index === state.lyric) return;
	state.lyric = index;
	$("lyrics-lines")
	...
	$("lyrics-lines")
		.querySelector(`[data-lyric="${index}"]`)
		?.scrollIntoView({
			block: "center",
			behavior: matchMedia("(prefers-reduced-motion: reduce)").matches
				? "auto"
	...
	state.lyricScrollTimer = setTimeout(() => {
		state.lyricAutoScrolling = false;
	}, 500);
}
...
async function libraryQueue(shuffle, currentKey = "", windowed = true) {
	...
	const keys = Array.isArray(result?.keys)
		? result.keys.filter((key) => typeof key === "string")
		: [];
...
async function toggleShuffle() {
	...
	const keys = await libraryQueue(
		enabled,
		enabled ? state.current?.key || "" : "",
	);
...
function updateModes() {
	localStorage.setItem("tm-shuffle", state.shuffle ? "1" : "0");
	localStorage.setItem("tm-repeat", state.repeat);
...
async function move(direction, ended = false) {
	...
	if ((next < 0 || next >= state.queue.length) && state.queueTruncated) {
		...
		const resolved = resolveWindowEdge(
			direction,
			state.queue,
			state.queueIndex,
			true,
		...
		if (resolved) {
			...
			state.queueIndex = resolved.queueIndex;
			renderQueue();
			next = resolved.next;
		}
	}
	if (next >= state.queue.length) {
		...
		if (state.shuffle) {
			state.queue = await libraryQueue(true, state.current?.key || "", false);
			next = 0;
		} else next = 0;
	}
	if (next < 0) next = state.repeat === "all" ? state.queue.length - 1 : 0;
	state.queueIndex = next;
	await playKey(state.queue[next]);
}

async function ensureSummaries(keys) {
	const missing = [...new Set(keys)]
		.filter(
			(key) => !state.summaryCache.has(key) && !state.summaryRequests.has(key),
		)
		.slice(0, 100);
	if (!missing.length) return;
	missing.forEach((key) => state.summaryRequests.add(key));
	try {
		const result = await api("/api/tracks/summaries", {
			method: "POST",
		...
		if (!error?.quiet) showError(error);
	} finally {
		missing.forEach((key) => state.summaryRequests.delete(key));
	}
}

const QUEUE_EMPTY =
	'<div class="queue-empty"><strong>Your queue is empty</strong><span>Choose a track or add one from its more menu.</span><button class="button" type="button" data-queue-browse>Browse library</button></div>';
...
async function schedulePrefetch() {
	...
	const count = Math.max(
		0,
		Math.min(Number(state.settings.prefetchCount) || 0, 20),
	);
	const keys = state.queue.slice(
		state.queueIndex + 1,
		state.queueIndex + 1 + count,
	);
	if (!keys.length) return;
	renderQueue();
	try {
		const job = await api("/api/playback/prefetch", {
			method: "POST",
		...
		watchJob(job, (current) => {
			...
			if (!$("queue-pane").hidden)
				for (const [key, value] of Object.entries(current.result || {})) {
					const badge = document.querySelector(
						`.queue-row[data-queue-key="${CSS.escape(key)}"] .cache-state`,
					);
					if (badge) {
						badge.className = `cache-state ${value}`;
						badge.textContent = value;
					}
				}
		});
	} catch (error) {
		if (!error?.quiet) showError(error, schedulePrefetch);
	}
}
	...
	const poll = async () => {
		try {
				...
				$("sync-copy").textContent =
					current.state === "queued"
						...
						: current.state === "complete"
							? "Sync complete"
							: `${current.found.toLocaleString()} tracks indexed · ${current.processed.toLocaleString()} files checked`;
				const announcement = active
					? "Sync started."
					: current.state === "complete"
						? "Sync complete."
						: current.error
							? `Sync failed. ${current.error}`
							: "";
			...
			if (["queued", "running"].includes(current.state))
				setTimeout(poll, current.processed ? 1800 : 1000);
			else if (["sync", "preview"].includes(current.kind)) {
				state.libraryCache.clear();
				await loadLibrary(true);
				if (current.state === "complete")
					toast(
						current.kind === "preview"
							? "Temporary source indexed"
							: "Source is up to date",
					);
				else if (current.error)
					showError(new AppError(current.error, true), () =>
						current.kind === "preview"
							? selectTemporary()
							: syncSource(current.chatId, current.mode === "full"),
					);
			} else if (
				current.kind === "enrich" &&
				current.state === "complete" &&
...
async function syncSource(chatId, full = false) {
	state.syncingIds.add(String(chatId));
	renderSources();
	try {
		const job = await api(`/api/sources/${encodeURIComponent(chatId)}/sync`, {
			method: "POST",
			body: JSON.stringify({ full }),
		});
		watchJob(job, (current) => {
			if (current && !["queued", "running"].includes(current.state)) {
				state.syncingIds.delete(String(current.chatId || chatId));
				renderSources();
			}
		});
	} catch (error) {
		state.syncingIds.delete(String(chatId));
		renderSources();
		showError(error, () => syncSource(chatId, full));
	}
}

async function syncAllSources() {
	state.sources.forEach((s) => state.syncingIds.add(String(s.chatId)));
	renderSources();
	try {
		...
		setTimeout(() => {
			state.syncingIds.clear();
			renderSources();
		}, 3500);
	} catch (error) {
		state.syncingIds.clear();
		renderSources();
		showError(error, syncAllSources);
	}
}
...
async function toggleSource(input) {
	const item = state.discovered.find(
		(value) => value.chatId === input.dataset.chat,
	);
	if (!item) return;
	...
	try {
		const result = selected
			? await api("/api/sources", {
					method: "POST",
					body: JSON.stringify({ chatId: item.chatId }),
				})
			: await api(`/api/sources/${encodeURIComponent(item.chatId)}`, {
					method: "PATCH",
		...
		item.pending = false;
		renderDiscovered();
		showError(error, () => toggleSource(input));
	}
}
...
async function keepTemporarySource() {
	...
	try {
		...
		if (
			state.temporaryJob?.jobId &&
			result.job?.jobId !== state.temporaryJob.jobId
		) {
			api(`/api/jobs/${encodeURIComponent(state.temporaryJob.jobId)}`, {
				method: "DELETE",
			}).catch(() => {});
...
async function unselectSources(ids) {
	if (
		...
		!(await confirmAction(
			"Remove from library?",
			"They’ll disappear from this player and stop syncing. Nothing will be deleted or left in Telegram.",
			"Remove from library",
		))
	)
		return;
	try {
		await api("/api/sources/bulk-select", {
			method: "POST",
			body: JSON.stringify({ chatIds: ids, selected: false }),
		});
		...
		toast(
			ids.length === 1
				? "Source removed from library"
				: `${ids.length} sources removed from library`,
		);
	} catch (error) {
		showError(error, () => unselectSources(ids));
	}
}
...
async function resetMetadata() {
	if (
		...
		!(await confirmAction(
			...
			"Telegram’s original metadata will become visible again.",
			"Reset",
		))
	)
		return;
	try {
		const updated = await api(mediaUrl(state.editing, "metadata"), {
			...
			body: JSON.stringify({
				set: {},
				clear: Object.keys(state.editing.overrides),
			}),
		});
		state.editing = updated;
...
async function applyCandidate(id) {
	try {
		const updated = await api(`${mediaUrl(state.editing, "metadata")}/apply`, {
			method: "POST",
			body: JSON.stringify({
				candidateId: id,
				coverQuality: $("cover-quality").value,
			}),
		});
		state.editing = updated;
		cacheSet(state.trackCache, updated.key, updated, 100);
		...
		state.libraryCache.clear();
		await loadLibrary(true);
		$("metadata-status").textContent = "Internet metadata applied locally.";
	} catch (error) {
		metadataFailed(error);
	}
}
...
function showPanel(tab = "lyrics", toggle = false) {
	if (
		toggle &&
		!$("now-panel").hidden &&
		document.querySelector(`#${tab}-tab.active`)
	)
		return closePanel();
	...
	panel.hidden = false;
	$("app-shell").classList.add("panel-open");
	...
	for (const name of ["lyrics", "queue", "details"]) {
	...
	requestAnimationFrame(() => pane.classList.add("pane-entering"));
...
function setNowHeaderCompact(header, compact) {
	...
	const targets = NOW_HEADER_MORPH.map((selector) =>
...
async function selectCachedSource(chatId) {
	try {
		...
		showError(error, () => selectCachedSource(chatId));
	}
}
		...
		[
			{
				...
				action: () =>
					source.selected === false
						? playKey(
								key,
								state.globalTracks.map((item) => item.key),
							)
						: playFromLibrary(key),
			},
			...(narrow
				? [
						{
							...
							action: () => toggleRowLike(key),
						},
						{
							label: `Duration ${formatTime(durationMs / 1000)}`,
							action: null,
						},
					]
				: []),
			{
				label: "Play next",
				action: () => {
					const at = Math.max(state.queueIndex + 1, 0);
					state.queue.splice(at, 0, key);
					renderQueue();
					schedulePrefetch();
					toast("Playing next");
				},
			...
			},
			{
				label: "Download",
				action: () => {
					location.href = mediaUrl({ key }, "download");
				},
			},
			{
				label: "Edit metadata",
				action: async () => openMetadata(await getTrack(key)),
			},
			{
				label: "Edit lyrics",
				action: async () => {
					if (state.current?.key !== key) await playKey(key);
					openLyricsEditor();
				},
			},
			source.selected === false
				? {
						label: "Add source",
						action: () => selectCachedSource(source.chatId),
					}
...
function updateRowLikeUi(key, liked) {
	document
		.querySelectorAll(`[data-row-like-key="${CSS.escape(key)}"]`)
		.forEach((button) => {
			button.classList.toggle("active", liked);
			button.setAttribute("aria-pressed", String(liked));
			button.setAttribute(
				"aria-label",
				`${liked ? "Unlike" : "Like"} ${button.getAttribute("aria-label")?.replace(/^(Unlike|Like) /, "") || "track"}`,
			);
			button
				.querySelector("use")
				?.setAttribute("href", liked ? "#i-heart-filled" : "#i-heart");
...
function trackRepresentations(key) {
	return representationsFor(key, {
		tracks: state.tracks,
		globalTracks: state.globalTracks,
...
function trackLikedState(key, representations = trackRepresentations(key)) {
	return likedState(key, {
		pending: rowLikeOperations.get(key),
		current: state.current,
		trackCache: state.trackCache,
		summaryCache: state.summaryCache,
		representations,
...
async function toggleTrackLike(key, { notify = false } = {}) {
	...
	const applyLiked = (liked) => {
		trackRepresentations(key).forEach((track) => {
			track.liked = liked;
		});
	...
	try {
		const updated = await api(`/api/tracks/${encodeURIComponent(key)}/like`, {
			method: "PATCH",
			body: JSON.stringify({ liked: requested }),
		});
		const canonical =
			typeof updated?.liked === "boolean" ? updated.liked : requested;
		const resolved = resolveLikeResponse(
			rowLikeOperations,
			key,
			operation,
			canonical,
		...
		if (notify) {
			toast(canonical ? "Added to Liked Songs" : "Removed from Liked Songs");
			schedulePersist();
		}
		...
		if (operation.baseline !== operation.desired)
			state.likedCount += operation.baseline ? 1 : -1;
		applyLiked(baseline);
		showError(error);
	}
}
...
async function saveCurrentToTelegram() {
	if (!state.current) return;
	const button = $("save-current-telegram");
	button.disabled = true;
	button.setAttribute("aria-busy", "true");
	try {
		await api(`${mediaUrl(state.current, "saved-messages")}`, {
			method: "POST",
		});
		toast("Sent to Saved Messages");
	} catch (error) {
		showError(error, saveCurrentToTelegram);
	} finally {
		button.disabled = false;
		button.removeAttribute("aria-busy");
	}
...
function renderFrequentContacts() {
	...
	if (!show) {
		$("frequent-list").innerHTML = "";
		return;
	}
	$("frequent-list").innerHTML = frequent
		.map(
			(contact) =>
				`<button class="frequent-contact" type="button" data-contact="${contact.id}" title="${escapeHtml(contact.name)}"><img class="source-avatar" src="${contact.avatarUrl}" data-avatar-fallback="${escapeHtml(initials(contact.name))}" alt="" loading="lazy"><span class="frequent-name">${escapeHtml(contact.name)}</span></button>`,
		)
		.join("");
}
...
function renderContacts() {
	...
	$("share-status").textContent = typed
...
async function openShare() {
	...
	$("contact-list").innerHTML =
		'<div class="list-skeleton"><span></span><span></span></div>';
	try {
		state.contacts = await api("/api/telegram/contacts");
		renderContacts();
	} catch (error) {
		$("share-dialog").close();
		showError(error, openShare);
	}
}
...
function queueShare(recipientId) {
	...
	$("share-dialog").close();
	if (pendingShare) {
		clearTimeout(pendingShare.timer);
		clearInterval(pendingShare.tick);
	}
	...
	pending.timer = setTimeout(async () => {
		...
		pendingShare = null;
		try {
			await api(`/api/tracks/${encodeURIComponent(key)}/share`, {
				method: "POST",
				body: JSON.stringify({ recipientId }),
			});
			toast("Shared on Telegram");
		} catch (error) {
			showError(error);
		}
	}, 5000);
	...
	pending.tick = setInterval(() => {
		...
		if (pending.cancelled || left <= 0 || $("toast").hidden)
			return clearInterval(pending.tick);
		$("toast-message").textContent =
			`Sharing in ${left} second${left === 1 ? "" : "s"}…`;
	}, 250);
}
...
const locateRowFor = (track) =>
	document.querySelector(
		`#track-list [data-track-key="${CSS.escape(track.key)}"]`,
	);
...
async function waitForLocateRow(track, token, timeout) {
	...
	while (performance.now() - started < timeout) {
		...
		if (!library.classList.contains("is-refreshing")) {
			...
			if (row) return row;
			if (!clearedFilter && search.value) {
				// filtered out: reveal once, then look again
				clearedFilter = true;
				search.value = "";
...
function installResizer(id, side) {
	...
	const resize = (clientX) => {
		...
		const bounded = Math.max(
			side === "left" ? 220 : 300,
			Math.min(side === "left" ? 420 : 640, value),
		);
		document.documentElement.style.setProperty(
			side === "left" ? "--rail-width" : "--panel-width",
			`${bounded}px`,
		);
		localStorage.setItem(
			side === "left" ? "tm-rail-width" : "tm-panel-width",
			bounded,
		);
	};
	...
	handle.addEventListener("keydown", (event) => {
		...
		resize(
			side === "left"
				...
				: innerWidth - current + (event.key === "ArrowLeft" ? 10 : -10),
		);
	});
}
...
function renderAccount(account) {
	...
	$("settings-account-name").textContent = linked
		? account.displayName || "Telegram account"
		: "Not connected";
	const parts = [];
...
async function openSettings() {
	...
	try {
		...
		$("prefetch-count").value = state.settings.prefetchCount;
		$("musicbrainz-contact").value = state.settings.musicbrainzContact;
		$("default-cover-quality").value = state.settings.coverQuality;
		$("auto-artwork").checked = state.settings.autoArtwork !== false;
		updateAutoArtworkHelp();
		const cache = await api("/api/cache/status");
		$("cache-usage").textContent =
			`${cache.files} songs cached · ${formatBytesShort(cache.bytes)}`;
		const [network, auth, status] = await Promise.all([
			api("/api/network"),
			api("/api/auth/status"),
			api("/api/status"),
		]);
...
function renderNetwork() {
	...
	for (const button of document.querySelectorAll(
		"[data-bind-host] [data-value]",
	)) {
		const active = button.dataset.value === bindHost;
		button.classList.toggle("active", active);
		button.setAttribute("aria-pressed", String(active));
	}
	$("bind-host-help").textContent =
		"This machine only keeps the player private. Anyone on my network lets other devices reach it — including anyone else on the same Wi-Fi.";
	...
	try {
		state.network = {
			...
			...(await api("/api/network", {
				method: "PATCH",
				body: JSON.stringify({ bindHost: value }),
			})),
		};
		renderNetwork();
		toast(
			value === "0.0.0.0"
				? "Now reachable on your network after restart"
				: "Now this machine only after restart",
		);
	} catch (error) {
...
$("password-remove").addEventListener("click", () =>
	showPasswordForm("disable"),
);
$("password-cancel").addEventListener("click", () => showPasswordForm(null));
$("password-disable-cancel").addEventListener("click", () =>
	showPasswordForm(null),
);
...
$("password-form").addEventListener("submit", async (event) => {
	...
	if (next.length < 8) {
		setFieldError($("password-new"), "Use at least 8 characters.");
		status.textContent = "Fix the highlighted field.";
		return;
	}
	if (next !== $("password-confirm").value) {
		setFieldError($("password-confirm"), "Those passwords do not match.");
		status.textContent = "Fix the highlighted field.";
		return;
	}
	...
	try {
		...
		if (/current/i.test(error.message))
			setFieldError($("password-current"), error.message);
	}
});
...
$("password-disable-form").addEventListener("submit", async (event) => {
	...
	try {
		...
		status.textContent = error.message;
		setFieldError($("password-disable-current"), error.message);
	}
});
...
$("country-search").addEventListener("focus", () => {
	if (!state.countriesLoaded) return;
	renderCountryOptions(selectedCountry ? "" : $("country-search").value);
});
$("country-search").addEventListener("keydown", (event) => {
	const open = !$("country-listbox").hidden;
	if (event.key === "Backspace" && selectedCountry) {
		// A committed country is one selectable token, not editable text: a single Backspace
		// clears the whole selection regardless of caret position.
		event.preventDefault();
		clearCountry({ focus: false });
	...
	if (event.key === "ArrowDown" || event.key === "ArrowUp") {
		...
		if (!open)
			return renderCountryOptions(
				selectedCountry ? "" : $("country-search").value,
			);
	...
	if (event.key === "Escape" && open) {
		...
		return closeCountryList();
	}
	if (event.key === "Tab" && open && countryActive >= 0)
		selectCountry(countryMatches[countryActive], { silent: true });
});
...
$("country-listbox").addEventListener("mousemove", (event) => {
	const option = event.target.closest(".combo-option");
	if (option && Number(option.dataset.index) !== countryActive) {
		countryActive = Number(option.dataset.index);
		syncCountryActive();
	}
});
...
$("source-list").addEventListener("click", (event) => {
	...
	if (menu) {
		...
		return openMenu(
			[
				{
					label: "Sync new tracks",
					action: () => syncSource(source.chatId, false),
				},
				{ label: "Full rescan", action: () => syncSource(source.chatId, true) },
				{
					label: source.pinnedAt ? "Unpin from top" : "Pin to top",
					action: () => pinSource(source.chatId),
				},
			],
			box.left,
			box.bottom + 4,
	...
	if (checkbox) {
		checkbox.checked
			? state.selectedSources.add(checkbox.dataset.bulkSource)
			: state.selectedSources.delete(checkbox.dataset.bulkSource);
...
$("source-list").addEventListener("keydown", (event) => {
	...
	if (row && ["Enter", " "].includes(event.key)) {
		event.preventDefault();
		selectSource(row.dataset.source);
	}
});
$("source-list").addEventListener("contextmenu", (event) => {
	const row = event.target.closest("[data-source]");
	if (row) {
		event.preventDefault();
		sourceMenu(row.dataset.source, event.clientX, event.clientY);
	}
});
function cleanupSourceDrag() {
	$("source-list")
		.querySelectorAll(".source-entry.is-dragging, .source-entry.drag-over")
		.forEach((el) => el.classList.remove("is-dragging", "drag-over"));
}
function closestSourceEntry(clientY) {
	const entries = [
		...$("source-list").querySelectorAll(".source-entry[draggable=true]"),
	];
...
$("source-list").addEventListener("dragover", (event) => {
	...
	$("source-list")
		.querySelectorAll(".source-entry.drag-over")
		.forEach((el) => el.classList.remove("drag-over"));
	const best = closestSourceEntry(event.clientY);
	if (best.entry && best.entry.dataset.source !== draggedSource)
		best.entry.classList.add("drag-over");
});
$("source-list").addEventListener("drop", async (event) => {
	...
	if (!draggedSource) return;
	const entries = [
		...$("source-list").querySelectorAll(
			".source-entry[draggable=true]:not(.is-dragging)",
		),
	];
	...
	if (!target || target === draggedSource) {
		draggedSource = "";
		return;
	}
	...
	try {
		await api("/api/sources/order", {
			method: "PATCH",
			body: JSON.stringify({ chatIds: ordered }),
		});
...
$("source-list").addEventListener("dragend", () => {
	cleanupSourceDrag();
	draggedSource = "";
});

$("track-list").addEventListener("click", (event) => {
	const play = event.target.closest("[data-play-key]");
	const menu = event.target.closest("[data-track-menu]");
	const like = event.target.closest("[data-row-like-key]");
	if (play) playFromLibrary(play.dataset.playKey).catch(showError);
	if (menu) {
		const rect = menu.getBoundingClientRect();
		trackMenu(menu.dataset.trackMenu, rect.right, rect.bottom);
	}
	if (like) {
		event.stopPropagation();
		toggleRowLike(like.dataset.rowLikeKey).catch(showError);
	}
});
$("track-list").addEventListener("focusin", (event) => {
	const row = event.target.closest(".track-row");
	if (row?.dataset.trackIndex != null)
		state.rowFocusIndex = parseInt(row.dataset.trackIndex, 10);
});
$("track-list").addEventListener("keydown", (event) => {
	...
	if (event.key === "End") {
		event.preventDefault();
		focusTrackRow(state.totalTracks - 1);
		return;
	}
...
$("track-list").addEventListener("contextmenu", (event) => {
	const row = event.target.closest("[data-track-key]");
	if (row) {
		event.preventDefault();
		trackMenu(row.dataset.trackKey, event.clientX, event.clientY);
	}
});
$("track-list").addEventListener(
	"error",
	(event) => {
		if (event.target.matches(".row-art")) {
			event.target.classList.remove("is-ready");
			event.target.nextElementSibling?.classList.remove("is-covered");
		}
	},
	true,
);
...
$("track-list").addEventListener(
	...
	(event) => {
		...
		if (image.matches(".row-art"))
			requestAnimationFrame(() => {
				image.classList.add("is-ready");
				image.nextElementSibling?.classList.add("is-covered");
			});
	},
	true,
);
...
libraryScroller().addEventListener(
	"scroll",
	() => {
		const scroller = libraryScroller();
		document
			.querySelector(".library-header")
			.classList.toggle("is-scrolled", scroller.scrollTop > 8);
		$("library").classList.add("is-scrolling");
		clearTimeout(scrollIdleTimer);
		scrollIdleTimer = setTimeout(finishLibraryScroll, 120);
...
$("global-search").addEventListener("focus", () =>
	renderGlobalSearch("Type a title, artist, or source name."),
);
$("global-search").addEventListener("input", () => {
	clearTimeout(globalSearchTimer);
	globalSearchTimer = setTimeout(searchEverywhere, 280);
});
...
$("global-search").addEventListener("keyup", (event) => {
	if (event.key === "Escape") closeGlobalSearch({ clear: true });
});
$("global-search-trigger").addEventListener("click", () => {
	if ($("app-shell").classList.contains("sidebar-collapsed")) {
		$("app-shell").classList.remove("sidebar-collapsed");
		localStorage.setItem("tm-sidebar", "expanded");
	}
	requestAnimationFrame(() => $("global-search").focus());
});
$("close-global-search").addEventListener("click", () =>
...
$("global-results").addEventListener("click", (event) => {
	...
	if (source) {
		const item = state.globalSources.find(
			(value) => value.chatId === source.dataset.globalSource,
		);
		if (item?.selected) selectSource(item.chatId).catch(showError);
			...
			selectTemporary().catch(showError);
		}
	}
	if (track) {
		const queue = state.globalTracks.map((item) => item.key);
		closeGlobalSearch();
		playKey(track.dataset.globalTrack, queue).catch(showError);
	}
});
$("global-results").addEventListener("contextmenu", (event) => {
	const track = event.target.closest("[data-global-track]");
	if (track) {
		event.preventDefault();
		trackMenu(track.dataset.globalTrack, event.clientX, event.clientY);
	}
});
$("play-playlist").addEventListener("click", () =>
	startPlaylist(false).catch(showError),
);
$("shuffle-playlist").addEventListener("click", () =>
	startPlaylist(true).catch(showError),
);
$("sync-source").addEventListener("click", () =>
	state.source
		? syncSource(state.source, false)
		: toast("Choose a source to sync"),
);
$("keep-source").addEventListener("click", () =>
	keepTemporarySource().catch(showError),
);
$("add-source").addEventListener("click", openSources);
document
	.querySelector('[data-action="add-source"]')
	.addEventListener("click", openSources);
$("sync-all-sources").addEventListener("click", () =>
	syncAllSources().catch(showError),
);
...
$("bulk-sources").addEventListener("click", () => {
	state.bulk = true;
...
$("bulk-unselect").addEventListener("click", () =>
	unselectSources([...state.selectedSources]),
);
...
$("play").addEventListener("click", () => togglePlayback().catch(showError));
$("previous").addEventListener("click", () =>
	audio.currentTime > 3 ? (audio.currentTime = 0) : move(-1).catch(showError),
);
$("next").addEventListener("click", () => move(1).catch(showError));
$("shuffle").addEventListener("click", () => toggleShuffle().catch(showError));
...
function setVolume(value) {
	...
	$("volume").setAttribute(
		"aria-valuetext",
		`${Math.round(audio.volume * 100)} percent`,
	);
	$("volume-toggle").classList.toggle("muted", !audio.volume);
	$("volume-toggle").setAttribute(
		"aria-label",
		audio.volume ? "Mute" : "Restore volume",
	);
...
$("volume-toggle").addEventListener("click", () =>
	setVolume(audio.volume ? 0 : lastAudibleVolume),
);
setVolume(localStorage.getItem("tm-volume") ?? 0.8);
...
audio.addEventListener("timeupdate", () => {
	updateProgress();
	updateLyric();
	const threshold = audio.duration ? Math.min(30, audio.duration / 2) : 30;
	if (
		state.current &&
		!state.current.qualified &&
		audio.currentTime >= threshold
	) {
		state.current.qualified = true;
		api("/api/playback/events", {
			method: "POST",
			body: JSON.stringify({ key: state.current.key, event: "qualified" }),
		}).catch(() => {});
	}
});
for (const event of ["progress", "durationchange", "loadedmetadata"])
...
if ("mediaSession" in navigator) {
	navigator.mediaSession.setActionHandler("play", () =>
		startAudioPlayback().catch(showError),
	);
...
$("queue-list").addEventListener("click", (event) => {
	...
	if (event.target.closest("[data-queue-browse]")) {
		closePanel();
		return $("track-list").querySelector(".track-row")?.focus();
	}
	if (play)
		playKey(
			state.queue[Number(play.dataset.queuePlay)],
			null,
			Number(play.dataset.queuePlay),
		).catch(showError);
	if (button) {
...
$("queue-list").addEventListener("dragstart", (event) => {
	draggedQueue = Number(
		event.target.closest("[data-queue-index]")?.dataset.queueIndex,
	);
	event.target.closest("[data-queue-index]")?.classList.add("queue-dragging");
});
$("queue-list").addEventListener("dragover", (event) => event.preventDefault());
$("queue-list").addEventListener("drop", (event) => {
	event.preventDefault();
	document.querySelector(".queue-dragging")?.classList.remove("queue-dragging");
	const target = Number(
		event.target.closest("[data-queue-index]")?.dataset.queueIndex,
	);
	if (
		Number.isInteger(draggedQueue) &&
		Number.isInteger(target) &&
		draggedQueue !== target &&
		draggedQueue > state.queueIndex &&
		target > state.queueIndex
	) {
		const [key] = state.queue.splice(draggedQueue, 1);
		state.queue.splice(target, 0, key);
		renderQueue();
		schedulePrefetch();
		schedulePersist();
	}
...
$("player-locate").addEventListener("pointerdown", () => {
	$("player-locate").dataset.pointerFocus = "true";
});
$("player-locate").addEventListener("blur", () => {
	delete $("player-locate").dataset.pointerFocus;
});
$("player-locate").addEventListener("click", () => locateCurrent());
$("show-lyrics").addEventListener("click", () => showPanel("lyrics", true));
$("close-now").addEventListener("click", closePanel);
for (const tab of ["lyrics", "queue", "details"])
	$(`${tab}-tab`).addEventListener("click", () => showPanel(tab));
$("now-panel")
...
$("lyrics-lines").addEventListener("click", (event) => {
	const line = event.target.closest("[data-lyric]");
	if (line) {
		audio.currentTime =
			state.lyrics.lines[Number(line.dataset.lyric)].startMs / 1000;
		state.lyricsFollow = true;
		$("sync-lyrics").hidden = true;
		line.classList.remove("seek-pulse");
		void line.offsetWidth;
		line.classList.add("seek-pulse");
	}
});
$("now-content").addEventListener("scroll", updateNowHeader, { passive: true });
$("now-panel").addEventListener("wheel", stopFollowingLyrics, {
	passive: true,
});
$("now-panel").addEventListener("touchmove", stopFollowingLyrics, {
	passive: true,
});
$("sync-lyrics").addEventListener("click", () => {
	state.lyricsFollow = true;
	$("sync-lyrics").hidden = true;
	state.lyric = -2;
	updateLyric();
});
$("add-lyrics-empty").addEventListener("click", openLyricsEditor);
$("edit-current").addEventListener("click", () => openMetadata());
$("edit-lyrics").addEventListener("click", openLyricsEditor);
$("like-current").addEventListener("click", () =>
	toggleLike().catch((error) => showError(error, toggleLike)),
);
$("save-current-telegram").addEventListener("click", saveCurrentToTelegram);
$("share-current").addEventListener("click", openShare);
$("player-more").addEventListener("click", (event) => {
	if (!state.current) return;
	const rect = event.currentTarget.getBoundingClientRect();
	trackMenu(state.current.key, rect.right, rect.top);
});
...
for (const id of ["contact-list", "frequent-list"]) {
	$(id).addEventListener("click", (event) => {
		...
		if (contact) queueShare(contact.dataset.contact);
	});
}
...
$("prefetch-count").addEventListener("change", () =>
	commitSettings(currentSettingsValues()),
);
$("sleep-timer").addEventListener("change", () =>
	setSleepTimer($("sleep-timer").value),
);
$("default-cover-quality").addEventListener("change", () =>
	commitSettings(currentSettingsValues()),
);
$("auto-artwork").addEventListener("change", () =>
	commitSettings(currentSettingsValues()),
);
...
$("musicbrainz-contact").addEventListener("input", () => {
	updateAutoArtworkHelp();
	saveSettingsSoon(currentSettingsValues());
});
...
$("test-musicbrainz").addEventListener("click", async () => {
	try {
		state.settings = await api("/api/settings", {
			method: "PATCH",
			body: JSON.stringify({
		...
		await api("/api/settings/musicbrainz/test", { method: "POST" });
		toast("MusicBrainz connection works");
	} catch (error) {
		showError(error, () => $("test-musicbrainz").click());
	}
});
$("clear-cache").addEventListener("click", async () => {
	if (
		await confirmAction(
			"Clear prefetched songs?",
			"Playback metadata and Telegram files will not be changed.",
			"Clear cache",
		)
	) {
		try {
			...
			toast("Prefetched songs cleared");
		} catch (error) {
			showError(error);
		}
	}
...
document.querySelectorAll("[data-setting] [data-value]").forEach((button) =>
	button.addEventListener("click", () => {
		localStorage.setItem(
			`tm-${button.parentElement.dataset.setting}`,
...
$("disconnect-telegram").addEventListener("click", async () => {
	if (
		await confirmAction(
			"Disconnect Telegram?",
			"This signs out the stored Telegram session and clears the local library. It does not leave or delete any Telegram chats.",
			"Disconnect",
		)
	) {
		try {
			await api("/api/telegram/session", { method: "DELETE" });
			location.reload();
		} catch (error) {
			showError(error);
		}
...
$("logout-telegram").addEventListener("click", async () => {
	if (
		await confirmAction(
			"Log out of Telegram?",
			"This signs this device out of Telegram. The indexed library, likes, lyrics, metadata edits and cached songs stay on this machine.",
			"Log out",
		)
	) {
		try {
...
$("context-menu").addEventListener("keydown", (event) => {
	...
	if (event.key === "Tab") {
		...
		event.preventDefault();
		const next =
			items[(index + (event.shiftKey ? -1 : 1) + items.length) % items.length];
		next.focus();
		return;
	}
	if (event.key === "ArrowDown") {
...
$("context-menu").addEventListener("click", (event) => {
	...
	if (button) {
		const action =
			$("context-menu")._actions[Number(button.dataset.menuIndex)]?.action;
		closeMenu();
		action?.();
	}
});
document.addEventListener("pointerdown", (event) => {
	if (!event.target.closest("#context-menu") && !$("context-menu").hidden)
		closeMenu();
	if (
		!event.target.closest(".global-search-wrap") &&
...
$("open-nav").addEventListener("click", () => {
	$("source-rail").classList.add("open");
...
addEventListener("pagehide", persistPlayerState);
addEventListener("beforeunload", persistPlayerState);
