...
test("the generated sprite contains every Hugeicons app icon", () => {
  const symbols = [...index.matchAll(/<symbol id="(i-[a-z-]+)" viewBox="0 0 24 24">/g)]
    .map((match) => match[1]);
  assert.deepEqual(symbols, iconIds.map((id) => `i-${id}`));
...
test("static and dynamic icon references resolve to the generated sprite", () => {
  ...
  const references = [...index.matchAll(/href="#(i-[a-z-]+)"/g)].map((match) => match[1]);
  for (const name of app.matchAll(/\bicon\(([^)]*)\)/g)) {
    for (const id of name[1].matchAll(/"([a-z-]+)"/g)) references.push(`i-${id[1]}`);
  }
