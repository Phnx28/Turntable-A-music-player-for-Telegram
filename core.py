...
class Database:
    ...
    def _migrate(self) -> None:
        # The numbered migration chain lives in migrations.py (Phase O1 step 1);
        # the rebuild callback is the one Database capability the chain needs.
        apply_migrations(self.connection, self._rebuild_search)
    ...
    def set_password(self, password: str) -> None:
        ...
        # The scrypt hash costs ~100 ms of CPU; compute it BEFORE opening the write
        # transaction so the database lock is never held for it (Phase O4). The
        # caller (an async route) wraps this whole call in to_thread.
